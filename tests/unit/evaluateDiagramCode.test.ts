...
describe('evaluateDiagramCode', () => {
  ...
  it('supports imported SDK entities and constructor syntax', async () => {
    const result = await evaluateDiagramCode(
      `import { ReactApp, User } from "@flowconsole/sdk";
       const user = new User({ name: "Analyst" });
       const app = new ReactApp({ name: "Portal" });
       user.sendsRequest(app, "open portal");`
    );
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.model.nodes.some((n) => n.data.title === 'Analyst')).toBe(true);
      expect(result.model.nodes.some((n) => n.data.title === 'Portal')).toBe(true);
      expect(result.model.edges.length).toBeGreaterThan(0);
    }
  }, 15000);
